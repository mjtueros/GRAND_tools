...
def interpolate_trace(t1, trace1, x1, t2, trace2, x2, xdes, upsampling=None,  zeroadding=None, ontrue=None, flow=60.e6, fhigh=200.e6):
    ...
    for i in range(0,len(phi)-1):
      ...
      if(np.abs(phi[i]-phi2[i])>np.pi):
        if(phi[i]<phi2[i]):
          ...
          phides[i]= (weight1 * phi[i] + weight2 * (phi2[i]+2*np.pi)) - 2*np.pi
    ...
    # re-unwrap: get -pi to +pi range back and check whether phidesis in between (im not wraping any more)
...
def do_interpolationonhdf5(desired, InputFilename, OutputFilename, antennamin=0, antennamax=159, EventNumber=0,shower_core=np.array([0,0,0]), DISPLAY=False, usetrace='efield'):
    '''
    ...
    positions_des = np.loadtxt(desired,usecols=(2,3,4))
...
def main():
    ...
    do_interpolationonhdf5(desired, hdf5file, OutputFilename, antennamin=0, antennamax=159, EventNumber=0,shower_core=np.array([0,0,2900]), DISPLAY=True,usetrace='voltage')
